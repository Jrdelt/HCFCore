...
public final class DeliveryStorage {
    ...
    public void enqueue(UUID owner,Collection<ItemStack> items,String source)throws SQLException{
        ...
        for(ItemStack item:items)if(item!=null&&!item.isEmpty())prepared.add(new PreparedDelivery(
                UUID.randomUUID().toString(),item));
        enqueuePrepared(owner,prepared,source);
    }
    ...
    public List<Reservation> delivering(UUID owner)throws SQLException{
        ...
        try(Connection connection=database.getConnection();PreparedStatement tokens=connection.prepareStatement("SELECT DISTINCT reservation_token FROM item_delivery_inbox WHERE owner_uuid=? AND state='DELIVERING' AND reservation_token IS NOT NULL ORDER BY reservation_token")){
            tokens.setString(1,owner.toString());try(ResultSet rs=tokens.executeQuery()){while(rs.next()){
                String token=rs.getString(1);try(PreparedStatement select=connection.prepareStatement("SELECT delivery_id,source,item FROM item_delivery_inbox WHERE owner_uuid=? AND reservation_token=? ORDER BY created_at,delivery_id")){
    ...
    public int complete(UUID owner,String token)throws SQLException{try(Connection connection=database.getConnection();PreparedStatement delete=connection.prepareStatement("DELETE FROM item_delivery_inbox WHERE owner_uuid=? AND state='DELIVERING' AND reservation_token=?")){delete.setString(1,owner.toString());delete.setString(2,token);return delete.executeUpdate();}}
