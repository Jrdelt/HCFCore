...
public final class FactionsHook {
    ...
    public static String getFactionTop(Player player) {
        ...
        return getFactionTopRanks().getOrDefault(faction.id(), "-");
    }

    /** Builds the shared power-rank snapshot once for consumers rendering many viewers. */
    public static Map<Integer, String> getFactionTopRanks() {
        ...
        for (int i = 0; i < factions.size(); i++) {
            ranks.put(factions.get(i).id(), String.valueOf(i + 1));
        }
        return ranks;
    }
    ...
    /** Finds a current faction id from a legacy tag stored before ids were persisted. */
    public static int getFactionIdByTag(String tag) {
        if (tag == null || tag.isBlank()) {
            return NO_FACTION;
        }
        for (Faction faction : Factions.factions().all()) {
            if (faction != null && faction.tag().equalsIgnoreCase(tag)) {
                return faction.id();
            }
        }
        return NO_FACTION;
    ...
    /** The immutable id of the faction claiming this location, or {@link #NO_FACTION} for wilderness. */
    public static int getClaimFactionId(Location location) {
        Faction faction = Board.board().factionAt(new FLocation(location));
        return faction == null || faction.isWilderness() ? NO_FACTION : faction.id();
    }
    ...
    /** The player's live faction, exposed for integrations that edit FactionsUUID's own permission matrix. */
