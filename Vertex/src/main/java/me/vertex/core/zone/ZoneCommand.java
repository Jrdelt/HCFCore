...
public final class ZoneCommand implements CommandExecutor, TabCompleter {
    ...
    private volatile ArenaControlManager arenaControls;
    ...
    public void setArenaControls(ArenaControlManager manager) { this.arenaControls = manager; }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sub.equals("create") || sub.equals("list")) return compactRegion(player, type, args);
    ...
    private boolean portal(Player player, ZoneType type, String[] args) {
        ...
        switch (args[1].toLowerCase(Locale.ROOT)) {
            case "create" -> {
                ...
                String result = manager.beginPortalSelection(player, args[2], target);
                player.sendMessage(messages.get(player,
                        "ok".equals(result) ? "portals.selection-started" : "portals.unknown-destination",
                        "name", args[2], "target", target.displayName()));
            }
            case "delete" -> player.sendMessage(messages.get(player,
                    args.length == 3 && manager.deletePortal(target, args[2]) ? "portals.deleted" : "portals.not-found"));
            case "list" -> {
                List<EntryPortal> configured = manager.portals().stream().filter(portal -> portal.target().equals(target)).toList();
            ...
            default -> player.sendMessage(messages.get(player, "zones.portal-create-usage", "command", type.configKey()));
        }
        return true;
    }
    ...
    private boolean spawnpoints(Player player, ZoneType type, String[] args) {
        ...
        if (action.equals("preview")) {
            ...
            player.sendMessage(messages.get(player, zones.routeBelongsTo(args[2], type) && zones.previewRoute(player, args[2])
                    ? "zones.route-preview" : "zones.route-invalid"));
            return true;
        }
        if (action.equals("delete")) {
            if (args.length < 3) return routeUsage(player, type);
            player.sendMessage(messages.get(player, zones.routeBelongsTo(args[2], type) && zones.deleteRoute(args[2])
    ...
    private boolean admin(Player player,ZoneType type,String[] args){if(!adminAllowed(player))return true;if(args.length<2){player.sendMessage(messages.get(player,"zones.admin-usage","command",type.configKey()));return true;}switch(args[1].toLowerCase(Locale.ROOT)){case "event"->{if(args.length>2&&args[2].equalsIgnoreCase("start")){zones.requestEventChange(player,true);}else if(args.length>2&&args[2].equalsIgnoreCase("stop")){zones.requestEventChange(player,false);}else player.sendMessage(messages.get(player,"zones.admin-unknown"));}case "inspect"->{Player target=args.length>2?Bukkit.getPlayerExact(args[2]):player;if(target!=null){player.sendMessage(messages.get(player,"zones.admin-inspect","player",target.getName(),"haven",String.valueOf(zones.kills(target,ZoneType.HAVEN)),"riftlands",String.valueOf(zones.kills(target,ZoneType.RIFTLANDS)),"amplification",String.valueOf(zones.amplification(target,zones.isIn(target,ZoneType.RIFTLANDS)?ZoneType.RIFTLANDS:ZoneType.HAVEN))));}else player.sendMessage(messages.get(player,"zones.admin-unknown"));}case "clear-mobs", "clearmobs" -> {boolean all=args.length>2&&args[2].equalsIgnoreCase("all");int removed=all?zones.clearAllZoneMobs():zones.clearZoneMobs(type);player.sendMessage(messages.get(player,"zones.admin-mobs-cleared","amount",String.valueOf(removed),"zone",all?"all":type.displayName()));}default->player.sendMessage(messages.get(player,"zones.admin-unknown"));}return true;}
    @Override public List<String> onTabComplete(CommandSender sender,Command command,String alias,String[] args){if(args.length==1)return complete(args[0],List.of("create","list","lootpool","portal","spawnpoints","koth","outpost","admin"));if((args[0].equalsIgnoreCase("koth")||args[0].equalsIgnoreCase("outpost"))&&args.length==2)return complete(args[1],List.of("create","cancel","delete","list"));if((args[0].equalsIgnoreCase("koth")||args[0].equalsIgnoreCase("outpost"))&&args.length==3&&args[1].equalsIgnoreCase("delete")&&arenaControls!=null){ArenaControlManager.ControlType pointType=args[0].equalsIgnoreCase("koth")?ArenaControlManager.ControlType.KOTH:ArenaControlManager.ControlType.OUTPOST;return complete(args[2],arenaControls.names(fixedType,pointType));}if(args[0].equalsIgnoreCase("create")&&args.length==2)return List.of();if(args[0].equalsIgnoreCase("portal")){if(args.length==2)return complete(args[1],List.of("create","delete","list"));if(args.length==3&&args[1].equalsIgnoreCase("delete")&&portals!=null){PortalTarget target=new PortalTarget(fixedType==ZoneType.HAVEN?PortalTarget.Kind.HAVEN:PortalTarget.Kind.RIFTLANDS,fixedType.configKey());return complete(args[2],portals.portals().stream().filter(portal->portal.target().equals(target)).map(EntryPortal::id).toList());}}if(args[0].equalsIgnoreCase("spawnpoints")){if(args.length==2)return complete(args[1],List.of("create","delete","preview","list"));if(args.length==3&&args[1].equalsIgnoreCase("create"))return complete(args[2],zones.regions(fixedType).stream().map(ZoneRegion::id).toList());if(args.length==3&&(args[1].equalsIgnoreCase("delete")||args[1].equalsIgnoreCase("preview")))return complete(args[2],zones.allRoutes().stream().map(ZoneRoute::id).toList());}if(args[0].equalsIgnoreCase("admin")&&args.length==2)return complete(args[1],List.of("event","inspect","clear-mobs"));if(args[0].equalsIgnoreCase("admin")&&args.length==3&&args[1].equalsIgnoreCase("event"))return complete(args[2],List.of("start","stop"));if(args[0].equalsIgnoreCase("admin")&&args.length==3&&args[1].equalsIgnoreCase("inspect"))return complete(args[2],Bukkit.getOnlinePlayers().stream().map(Player::getName).toList());if(args[0].equalsIgnoreCase("admin")&&args.length==3&&args[1].equalsIgnoreCase("clear-mobs"))return complete(args[2],List.of("all"));return List.of();}
