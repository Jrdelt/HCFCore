...
public final class KitCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("save")) {
            if (!player.hasPermission("vertex.kit.create")
                    && !player.hasPermission("vertex.kit.save")) {
                player.sendMessage(messages.getChat(player, "general.no-permission"));
                return true;
            }
            if (args.length < 2) {
            ...
            String name = args[1];
            String permission = args.length > 2 ? args[2] : "vertex.kit." + name.toLowerCase(Locale.ROOT);
